...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    #distance=self.distancer.getDistance(pacmanPosition,livingGhosts)
    #successorPosition = Actions.getSuccessor(position,action)

    
    #find closest ghost
    #capture the most likely position of each ghost that hasn't been captured
    ghost_closer=0
    mindis=float("inf")
    for p in livingGhostPositionDistributions:
        #livingGhostPositionDistributions is a list of util.Counter
        #argMax in util returns the key with the highest value
        #print 'pppp',p.argMax()
        pacman=pacmanPosition
        ghost=p.argMax()
        pacghodis=self.distancer.getDistance(ghost,pacman)
        ...
        if pacghodis>=mindis:
            ...
            a=None
            for a in legal:
                nextpos=Actions.getSuccessor(pacman,a)
                pacdis1=self.distancer.getDistance(nextpos,ghost_closer)
                if pacdis1<mindis:
                    mindis=pacdis1
                    move=a
            return move
